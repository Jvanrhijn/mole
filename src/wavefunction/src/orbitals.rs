    ...
    T: BasisSet,
{
    pub fn new(parms: Array2<f64>, basis_set: T) -> Self {
        ...
        Self {
            parms: parms.into_shape(ncenters * nwidths).unwrap(),
            basis_set,
            ncenters,
            nwidths,
        }
    ...
    T: BasisSet,
{
    ...
    fn value(&self, cfg: &Array<f64, Self::D>) -> Result<f64, Error> {
        let parms =
            Array2::from_shape_vec((self.ncenters, self.nwidths), self.parms.to_vec()).unwrap();
    ...
    T: BasisSet,
{
    fn parameter_gradient(&self, cfg: &Array2<f64>) -> Result<Array1<f64>, Error> {
        ...
        Ok(self
            .basis_set
            .coefficient_derivative(&cfg.clone().into_shape(3)?, &parms)
            .into_shape(self.ncenters * self.nwidths)?)
    }
