...
use rand::{StdRng, SeedableRng, FromEntropy, RngCore};
        ...
        <R as SeedableRng>::Seed: From<[u8; 32]>,
{
    ...
    pub fn diffuse(&mut self, time_step: f64, num_iterations: usize, equilibration: usize) -> (Vec<f64>, Vec<f64>) {
        ...
        for j in 0..num_iterations {
            ...
            let mut confs_weighted: Vec<_> = self.walkers.iter()
            ...
            if j > equilibration {
                ...
                energies.push(dmc_energy_prev + (self.reference_energy - dmc_energy_prev)/ (j - equilibration + 2) as f64);
                vars.push(vars.last().unwrap() + ((self.reference_energy - dmc_energy_prev)*(self.reference_energy - energies.last().unwrap()) - *vars.last().unwrap())
                    /(j - equilibration + 1) as f64);
                println!("Reference Energy:   {:.8}    DMC Energy:   {:.8} +/- {:.8}", self.reference_energy, *energies.last().unwrap(), vars.last().unwrap().sqrt() / ((j - equilibration) as f64).sqrt());
            }
